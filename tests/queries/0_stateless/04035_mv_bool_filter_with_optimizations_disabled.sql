-- Regression test for #83894:
-- Wrong count from materialized view with `query_plan_enable_optimizations = 0`
--
-- When a MaterializedView maps an Int column to a Bool column, a filter on the
-- Bool column must not be pushed to the underlying storage using Bool semantics
-- as if it were an exact key lookup. Bool `true` means "any non-zero value",
-- so -1 and -2 both satisfy `c0 = TRUE`.
